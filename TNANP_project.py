...
if do_all_regimes_EoS_specific_p0: #In this section I intend to treat the same equation of state but now with a specifica initial pressure
    ...
    P0 = 0.01 #Specific choosen dimensionless initial central pressure

    dimless_central_pressures = np.array([P0]) #
    ...
    #Now we only have one key in the dictionary that correponds to the dimless central pressure p0 = P0

    #Build radius arrays to plot mass and pressure vs radius to study how these behave:
    newton_radius_array = np.linspace(0, (len(newton_masses[P0])-1)/100, len(newton_masses[P0]))
    TOV_radius_array = np.linspace(0, (len(TOV_masses[P0])-1)/100, len(TOV_masses[P0]))
    
    plt.figure()
    plt.plot(newton_radius_array, newton_dimless_pressures[P0], label = f'$p_0$ = P0 Newton')
    plt.plot(TOV_radius_array, TOV_dimless_pressures[P0], label = f'$p_0$ = P0 TOV')
    ...
    plt.figure()
    plt.plot(newton_radius_array, newton_masses[P0], label = f'$p_0$ = P0 Newton')
    plt.plot(TOV_radius_array, TOV_masses[P0], label = f'$p_0$ = P0 TOV')
    plt.ylabel('M in $M_\u2609$')
    plt.xlim(0,15)
    plt.ylim(0,1.1)
    plt.xlabel('R in km')
    ...
    print(f'Newton radius with p0 = {P0}: ', newton_final_radii[P0])
    print(f'Newton mass with p0 = {P0}: ', newton_masses[P0][-1])
    print(f'TOV radius with p0 = {P0}: ', TOV_final_radii[P0])
    print(f'TOV mass with p0 = {P0}: ', TOV_masses[P0][-1])
